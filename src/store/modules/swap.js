...
export default {
	...
	actions: {
		async loadDenoms({ dispatch, commit, getters, rootGetters }) {
			...
			if (!getLiquidityPools) {
				throw new Error('tendermint.liquidity.v1beta1/getLiquidityPools getter does not exist')
			}

			const { pools } = getLiquidityPools()

			const denoms = pools.reduce(
				(result, { reserve_coin_denoms, id: pool_id }) => ({
					...result,
					...reserve_coin_denoms.reduce(
						(acc, denom, index) => ({
							...acc,
							[denom]: {
								...(result[reserve_coin_denoms[index]] ? result[reserve_coin_denoms[index]] : {}),
								[index === 0 ? reserve_coin_denoms[1] : reserve_coin_denoms[0]]: pool_id,
							},
						}),
						{},
					),
	...
	getters: {
		...
		getDenomPairs: ({ denoms }) => (denom) => {
			...
			return Object.keys(denoms[denom])
				.sort()
				.map((changeTo) => ({
					pair: `${denom}/${changeTo}`,
					denom,
		...
		getPairPoolId: ({ denoms, dispatch }) => (denom1, denom2) => denoms[denom1][denom2],
		// not sure that it can be useful
		getAllPossiblePairs: ({ denoms }, { getAllDenomsNames, getDenomPairs }) => getAllDenomsNames.reduce((acc, denom) => [...acc, ...getDenomPairs(denom)], []),
	},
}
